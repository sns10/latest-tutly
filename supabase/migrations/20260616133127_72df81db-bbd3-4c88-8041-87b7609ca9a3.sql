...
BEGIN
  ...
  WITH raw_input AS (
    SELECT
      (rec->>'student_id')::uuid                AS rec_student_id,
      (rec->>'date')::date                       AS rec_date,
       rec->>'status'                            AS rec_status,
       NULLIF(rec->>'notes', '')                 AS rec_notes,
       NULLIF(rec->>'subject_id', '')::uuid      AS rec_subject_id,
       NULLIF(rec->>'faculty_id', '')::uuid      AS rec_faculty_id,
       ordinality                                AS rec_pos
    FROM jsonb_array_elements(_records) WITH ORDINALITY AS t(rec, ordinality)
  ),
  deduped AS (
    -- Keep only the last occurrence of each (student, date, subject, faculty)
    -- to avoid "ON CONFLICT DO UPDATE command cannot affect row a second time".
    SELECT DISTINCT ON (
      rec_student_id, rec_date,
      COALESCE(rec_subject_id, '00000000-0000-0000-0000-000000000000'::uuid),
      COALESCE(rec_faculty_id, '00000000-0000-0000-0000-000000000000'::uuid)
    )
      rec_student_id, rec_date, rec_status, rec_notes, rec_subject_id, rec_faculty_id
    FROM raw_input
    ORDER BY
      rec_student_id, rec_date,
      COALESCE(rec_subject_id, '00000000-0000-0000-0000-000000000000'::uuid),
  ...
  upserted AS (
    ...
    SELECT rec_student_id, rec_date, rec_status, rec_notes, rec_subject_id, rec_faculty_id
    FROM deduped
    ON CONFLICT (
      student_id,
      date,
  ...
  SELECT
    u.id, u.student_id, u.date, u.status, u.notes,
    u.subject_id, u.faculty_id, u.created_at, u.updated_at
  FROM upserted u;
END;
$$;
